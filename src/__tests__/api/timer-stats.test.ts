...
import { GET } from "@/app/api/timers/[id]/stats/route";

describe("GET /api/timers/[id]/stats", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  const callGET = (timerId: string, query = "", userId = "owner1") =>
    GET(
      createGetRequest(
        `/api/timers/${timerId}/stats${query ? `?${query}` : ""}`,
        { "x-user-id": userId }
      ),
      { params: Promise.resolve({ id: timerId }) }
    );

  it("타이머 미존재 시 404", async () => {
  ...
  it("소유자가 아닌 경우 403", async () => {
    const db = createMockDB();
    db._stmt.first.mockResolvedValueOnce({ id: "timer1", owner_user_id: "owner1" });
    vi.mocked(getDB).mockResolvedValue(db as unknown as D1Database);

    const res = await callGET("timer1", "", "other-user");
    expect(res.status).toBe(403);
    const json = await parseJson(res);
    expect(json.error.code).toBe("FORBIDDEN");
  });

  it("타이머 존재 + 로그 없음 → 200 + 0 값", async () => {
    const db = createMockDB();
    db._stmt.first
    ...
    const res = await callGET("timer1");
  ...
  it("타이머 존재 + 로그 있음 → 200 + 정상 데이터", async () => {
    const db = createMockDB();
    db._stmt.first
      .mockResolvedValueOnce({ id: "timer1", owner_user_id: "owner1" })
      .mockResolvedValueOnce({
  ...
  it("donorLimit 파라미터 적용", async () => {
    ...
    await callGET("timer1", "donorLimit=5");
  ...
  it("donorLimit 최대 50 제한", async () => {
    ...
    await callGET("timer1", "donorLimit=100");
