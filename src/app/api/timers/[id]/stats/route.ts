...
export const GET = withErrorHandler(async (
  ...
  const timer = await db
    .prepare(
      "SELECT t.id, p.owner_user_id FROM timers t JOIN projects p ON p.id = t.project_id WHERE t.id = ? AND t.status != 'DELETED'"
    )
    .bind(timerId)
